...
export const useExpandedSubGroups = (columnCount) => {
    const { groupingService: { groupingKeys, dateVariant, timeInterval }, } = useGarden();
    const [expanded, setExpanded] = useState(new Array(columnCount).fill(0).map(() => []));
    useEffect(() => {
        setExpanded(new Array(columnCount).fill(0).map(() => []));
    }, [groupingKeys.toString(), dateVariant, timeInterval, columnCount]);
    const expandColumn = (columnIndex, item) => {
    ...
    const collapseColumn = (columnIndex, subgroupName) => {
        const expandedIndexes = expanded[columnIndex];
