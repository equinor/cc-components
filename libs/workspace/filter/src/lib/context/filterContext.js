...
export const FilterContextProvider = ({ children, styles, initialState, dataSource, onChange, appName, }) => {
    const client = useRef(new QueryClient({ defaultOptions: { queries: { refetchOnWindowFocus: false } } }));
    return (_jsx(QueryClientProvider, { client: client.current, children: _jsx(FilterContextWrapper, { dataSource: dataSource, initialState: initialState, onChange: onChange, appName: appName, children: children }) }));
};
export const FilterContextWrapper = ({ dataSource, initialState, children, onChange, appName, }) => {
    ...
    const getPersistedGroupOrder = () => {
        const storedOrder = localStorage.getItem(LOCAL_STORAGE_KEY);
        return storedOrder ? JSON.parse(storedOrder) : [];
    };
    const getPersistedQuickFilters = () => {
        const storedFilters = localStorage.getItem(QUICK_FILTERS_KEY);
        return storedFilters ? JSON.parse(storedFilters) : undefined;
    };
    const getPersistedIncludeCount = () => {
        const storedValue = localStorage.getItem(INCLUDE_COUNT_KEY);
        return storedValue ? JSON.parse(storedValue) : false;
    };
    const [filterState, setFilterState] = useState(initialState?.filterState ?? { groups: [], search: '' });
    const [selectedItems, setSelectedItems] = useState({});
    const [quickFilters, setQuickFilters] = useState(getPersistedQuickFilters());
    const [includeCount, setIncludeCount] = useState(getPersistedIncludeCount());
    ...
    useEffect(() => {
        setSelectedItems(initialState?.filterState.groups.reduce((acc, group) => {
            acc[group.name] = group.values;
            return acc;
        }, {}) || {});
    }, []);
    const saveGroupOrderToLocalStorage = (order) => {
        localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(order));
    };
    const saveQuickFiltersToLocalStorage = (filters) => {
        localStorage.setItem(QUICK_FILTERS_KEY, JSON.stringify(filters));
    };
    const saveIncludeCountToLocalStorage = (include) => {
    ...
    const filterValues = Array.isArray(query.data)
        ...
        : [];
    const reorderFilterGroups = useCallback((newOrder) => {
    ...
    const setFilter = (filterName, values) => {
    ...
    useEffect(() => {
        const activeFilters = Object.keys(selectedItems)
            ...
            .map((key) => {
            return { name: key, values: selectedItems[key] };
        });
        setFilterState((v) => {
    ...
    const clearFilters = () => {
        if (!Object.values(selectedItems).some((sI) => sI.length > 0))
            return;
        setFilterState({ search: '', groups: [] });
        setSelectedItems({});
    };
    const updatePersistedQuickFilters = (filterName, isQuickFilter) => {
    ...
    const updatePersistedIncludeCount = (include) => {
