      ...
      >
        <FilterListContainer>
          {filterValues.map((group) => {
                ...
                ? [
                    {
                      ...
                      indeterminate:
                        filteredItems.some((item) => item.selected) && !filteredItems.every((item) => item.selected),
                      onClick: () => {
            ...
            return (
              <FilterContainer key={group.name} ref={containerRef}>
                  ...
                  {isSearchActive ? (
                    <SearchbarContainer>
                      ...
                      <Tooltip title={`Stop searching`}>
                        <HeaderIconButton variant="ghost_icon" onClick={handleToggleSearch} aria-label="Toggle search">
                          <SmallCloseIcon color={tokens.colors.interactive.primary__resting.hex}></SmallCloseIcon>
                        </HeaderIconButton>
                      </Tooltip>
                    </SearchbarContainer>
                  ) : (
                    <>
                      <Typography group="table" variant="cell_header" color={isFilterActive ? 'primary' : undefined}>
                        {group.name +
                      ...
                      <div>
                        ...
                        {group.filterItems.length >= MIN_ITEMS_FOR_SEARCH && (
                          <Tooltip title={`Search for ${group.name}`}>
                            ...
                            >
                              <SearchIcon color={tokens.colors.interactive.primary__resting.hex}></SearchIcon>
                            </HeaderIconButton>
                          </Tooltip>
                        )}
                      </div>
                      ...
                      return (
                        ...
                        >
                          <ListItem
                            ...
                            start={start}
                            height={size}
                            onClick={() => (item.onClick ? item.onClick() : handleFilterItemClick(group, item))}
                          >
                            <SmallCheckbox checked={item.selected} indeterminate={item.indeterminate} readOnly />
