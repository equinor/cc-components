...
function createBasicFilter(filters: FilterConfig | undefined): undefined | IBasicFilter {
  ...
  return {
    ...
    filterType: 1,
    operator: 'In',
    values: filters.values.filter((value): value is string => typeof value === 'string'),
  };
}
...
export const powerBiModule: FusionWorkspaceModule = {
  ...
  setup: (props) => {
    ...
    const filters = createBasicFilter(powerBiConfig.filters);
    const controller = new PowerBiController(filters ? [filters] : undefined);
      ...
      tab: {
        ...
        CustomHeader: () => (
          <PowerBiHeader
            ...
            usePowerBiFilters={props.usePowerBiFilters}
          />
        ),
        name: 'powerbi',
        TabIcon: () => <PowerBiIcon />,
  ...
  useEffect(() => {
    ...
    const unsub = powerBiConfig.controller.onReportReady((embed) => {
      ...
      const handler = async () => {
        ...
        Object.assign(window, { pbi: embed, bookmark: bookmarkPayload });
        updatePayload((old) => {
